...
[![MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE) [![tests](https://img.shields.io/badge/tests-265%20passing-brightgreen)]() [![Node](https://img.shields.io/badge/node-%E2%89%A522-blue)]()
...
One Node process, zero runtime dependencies (storage is `node:sqlite`, built into Node ≥ 22). Plain HTTP, real streaming, an in-process React dashboard, and 265 tests that run with `npm test` on any machine — no API keys required (the smoke test uses bundled stub providers).
